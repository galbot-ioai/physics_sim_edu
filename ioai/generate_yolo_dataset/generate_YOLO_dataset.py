...
######################################################################################
#
# Description: YOLO segmentation dataset generation framework for IOAI environment
# Author: Ziyue Cao@Galbot
#
######################################################################################
...
# Use tempfile to create a temporary dataset directory
def get_dataset_root():
    """Get temporary dataset root directory"""
    temp_dir = tempfile.mkdtemp(prefix="yolo_dataset_")
    return temp_dir


# Initialize dataset root
...
def cleanup_dataset():
    ...
    try:
        if os.path.exists(ROOT):
            shutil.rmtree(ROOT)
            print(f"[INFO] Cleaned up temporary dataset directory: {ROOT}")
    except Exception as e:
        print(f"[WARNING] Failed to clean up dataset directory: {e}")
...
def random_pose(x_range, y_range, z_value, object_name=None):
    ...
    if object_name == "bin":
        ...
        base_x = table_x - table_width / 2 + 0.3  # 30cm from left edge
        base_y = table_y + table_length / 2 - 0.2  # 20cm from bottom edge
        ...
        # Small random rotation (within 10 degrees)
        random_angle_deg = random.uniform(-10, 10)
        angle_rad = np.radians(random_angle_deg)
...
def generate_non_colliding_positions(
    object_list, x_range, y_range, z_value, max_attempts=50
):
...
def check_objects_intersection(
    synthnova_physics_simulator, object_geoms_dict, object_poses_dict, min_distance=0.01
):
    ...
    for i, object_name1 in enumerate(object_names):
        for j, object_name2 in enumerate(object_names):
            ...
            if edge_distance < min_distance:
                print(
                    f"[WARNING] Intersection: {object_name1} and {object_name2} (distance: {edge_distance:.3f}m)"
                )
                return True
...
def collect_and_save_sample_yolo_single(
    img_name,
    label_name,
    save_split_dir,
    galbot_interface,
    ...
    # Calculate segmentation labels based on mask - target object has specific geom_ids, background is 0
...
def collect_and_save_sample_yolo_multi(
    img_name,
    label_name,
    save_split_dir,
    galbot_interface,
...
def setup_simulator():
    ...
    sim_config = PhysicsSimulatorConfig(mujoco_config=MujocoConfig(headless=True))
    ...
    front_head_rgb_camera_config = RgbCameraConfig(
        ...
        translation=[0.10084319533055261, -0.059042081352783105, 0.03184978861787491],
        rotation=[
            -0.1654571792421115,
            0.6935589352367344,
            0.16457378953789606,
    ...
    front_head_rgb_camera_path = synthnova_physics_simulator.add_sensor(
        front_head_rgb_camera_config
    )
...
def setup_interface(
    ...
    galbot_interface.leg.set_joint_positions(
        [
            0.0821758285164833,
            0.6340972781181335,
            0.5227039456367493,
            -0.00001198422432935331,
        ],
        immediate=True,
    )
...
def main():
    ...
    parser.add_argument(
        "--start_idx", type=int, default=1, help="Start index (default: 1)"
    )
    parser.add_argument(
        "--end_idx", type=int, default=31, help="End index (default: 31)"
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    if args.object and args.object not in valid_objects:
        print(
            f"Error: Object name '{args.object}' not in supported list: {valid_objects}"
        )
        return
    ...
    if args.split:
        if args.split not in ["train", "test", "val"]:
            print(
                f"Error: Dataset split '{args.split}' invalid, should be train, test, val"
            )
            return
    ...
    safety_margin2 = 0.35
    x_range = (
        table_x - table_width / 2 + safety_margin1,
        table_x + table_width / 2 - safety_margin1,
    )
    y_range = (
        table_y - table_length / 2 + safety_margin1,
        table_y + table_length / 2 - safety_margin2,
    )
    z_value = 0.55
        ...
        for split in target_splits:
            ...
            for i in indices:
                ...
                while not success:
                    ...
                    position, quat = random_pose(x_range, y_range, z_value, object_name)
                    unique_prim_path = (
                        f"/World/{object_name[0].upper() + object_name[1:]}"
                        if object_name != "cube"
                        else f"/World/cube"
                    )
                    ...
                    if object_name == "cube":
                        ...
                        object_config = MeshConfig(
                            ...
                            mjcf_path=Path()
                            .joinpath(
                                synthnova_physics_simulator.synthnova_assets_directory
                            )
                            .joinpath("synthnova_assets")
                            .joinpath("objects")
                    ...
                    galbot_interface = setup_interface(
                        synthnova_physics_simulator,
                        robot,
                        front_head_rgb_camera_path,
                        front_head_depth_camera_path,
                    ...
                    geom_ids = [
                        synthnova_physics_simulator.model.geom_name2id(name)
                        for name in geom_names
                    ]
                    obj_state = synthnova_physics_simulator.get_object_state(
                        unique_prim_path
                    )
                    ...
                    if check_objects_on_table(
                        ...
                        collect_and_save_sample_yolo_single(
                            img_name,
                            label_name,
                            split,
                            galbot_interface,
                        ...
                        print(
                            f"[INFO] {object_name} {split} collected {i}/{len(indices)} frames"
                        )
                        ...
                        print(
                            f"[WARNING] {object_name} {split} frame {i} - object fell off table, preparing to retry"
                        )
        ...
        for split in target_splits:
            ...
            for i in indices:
                ...
                while not success:
                    ...
                    object_list = generate_random_object_list(
                    ...
                    print(
                        f"[INFO] {combination_name} {split} frame {i} - random objects: {object_list}"
                    )
                    ...
                    positions, orientations = generate_non_colliding_positions(
                        object_list, x_range, y_range, z_value
                    )

                    # Add objects with pre-generated non-colliding poses
                    for idx, object_name in enumerate(object_list):
                        unique_prim_path = (
                            f"/World/{object_name[0].upper() + object_name[1:]}_{idx}"
                            if object_name != "cube"
                            else f"/World/cube_{idx}"
                        )
                    ...
                    if check_objects_on_table(
                        ...
                        for object_name, geom_ids in object_geoms_dict.items():
                            if (
                                "bin" in object_name
                                and object_name in object_poses_dict
                            ):
                                ...
                                if (
                                    ...
                                    print(
                                        f"[WARNING] Bin {object_name} is outside expected range (x: {pos[0]:.3f}, y: {pos[1]:.3f})"
                                    )
                                    bin_outside = True
                                    break
                        ...
                        if check_objects_intersection(
                            ...
                            print(
                                f"[WARNING] {combination_name} {split} frame {i} - object intersection detected, preparing to retry"
                            )
                            synthnova_physics_simulator.close()
                            retry_count += 1
                            continue
                        ...
                        collect_and_save_sample_yolo_multi(
                            img_name,
                            label_name,
                            split,
                            galbot_interface,
                        ...
                        print(
                            f"[INFO] {combination_name} {split} collected {i}/{len(indices)} frames"
                        )
                        success = True
                    else:
                        # Some objects fell off table, close simulator and prepare to retry
                        print(
                            f"[WARNING] {combination_name} {split} frame {i} - some objects fell off table, preparing to retry"
                        )
                        synthnova_physics_simulator.close()
                        retry_count += 1
                        continue
