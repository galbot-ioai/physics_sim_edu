...
class IoaiGraspEnv:
    ...
    def _setup_simulator(self, headless=False):
        ...
        # Calculate handle positions relative to bin
        self.bin_handle_offsets = [
            [0.16, 0, 0.23],   # Handle 1: x=0.15, z=0.2
            [-0.16, 0, 0.23]   # Handle 2: x=-0.15, z=0.2
        ]
    ...
    def get_bin_handle_poses(self, handle_index: int = None) -> List[Tuple[np.ndarray, np.ndarray]]:
        """Get bin handle poses in world frame.
        
        Args:
            handle_index: Index of handle (0 or 1). If None, returns both handles.
            
        Returns:
            List of (position, orientation) tuples in world frame.
            If handle_index is specified, returns list with single element.
        """
        from scipy.spatial.transform import Rotation
        
        # Get bin orientation as rotation matrix
        bin_rot = Rotation.from_quat(self.bin_orientation)
        
        handle_poses = []
        
        # Determine which handles to process
        if handle_index is not None:
            handle_indices = [handle_index]
        else:
            handle_indices = [0, 1]  # Both handles
        
        for idx in handle_indices:
            if idx >= len(self.bin_handle_offsets):
                raise ValueError(f"Handle index {idx} out of range. Available handles: 0-{len(self.bin_handle_offsets)-1}")
            
            # Get relative offset for this handle
            relative_offset = np.array(self.bin_handle_offsets[idx])
            
            # Transform relative offset to world frame
            world_offset = bin_rot.apply(relative_offset)
            handle_position = self.bin_position + world_offset
            
            # Handle orientation is same as bin orientation
            handle_orientation = self.bin_orientation.copy()
            
            handle_poses.append((handle_position, handle_orientation))
        
        return handle_poses

    def get_bin_handle_pose_in_robot_frame(self, handle_index: int = 0) -> Tuple[np.ndarray, np.ndarray]:
        """Get bin handle pose in robot base frame.
        
        Args:
            handle_index: Index of handle (0 or 1)
            
        Returns:
            Tuple of (position, orientation) in robot base frame
        """
        # Get handle pose in world frame
        handle_poses = self.get_bin_handle_poses(handle_index)
        if not handle_poses:
            raise ValueError(f"No handle found for index {handle_index}")
        
        world_position, world_orientation = handle_poses[0]
        
        # Transform to robot base frame
        robot_position, robot_orientation = self.world_to_robot_frame(world_position, world_orientation)
        
        return robot_position, robot_orientation

    def bin_callback(self):
        """Callback for bin detection"""
    
        def state_1():
            # Get both handle poses in world frame
            if self.state_first_entry:
                robot_pos, robot_ori = self.get_bin_handle_pose_in_robot_frame(handle_index=0)
                
                # Move left arm to handle 0
                self._move_left_arm_to_pose(
                    target_position=robot_pos + [-0.15, 0, 0.1],
                    target_orientation=[0, 0, 0, 1]
                )

                robot_pos, robot_ori = self.get_bin_handle_pose_in_robot_frame(handle_index=1)

                # Move right arm to handle 1
                self._move_right_arm_to_pose(
                    target_position=robot_pos + [-0.15, 0, 0.1],
                    target_orientation=[0, 0, 0, 1]
                )
                self.state_first_entry = False

            return  not self.simulator.physics_callback_exists("LeftArm_follow_trajectory_callback") and not self.simulator.physics_callback_exists("RightArm_follow_trajectory_callback")
            # return False
        def state_2():
            if self.state_first_entry:
                robot_pos, robot_ori = self.get_bin_handle_pose_in_robot_frame(handle_index=0)
        ...
        self.state_machine.add_state("State 1", state_1)
        self.state_machine.add_state("State 2", state_2)
...
if __name__ == "__main__":
    ...
    env.simulator.add_physics_callback(
        "bin_callback",
        env.bin_callback
    )
    env.simulator.loop()
